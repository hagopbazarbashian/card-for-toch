...
@section('home')
<!--bread-crumb-->   
<div class="iq-breadcrumb" style="background-image: url({{ asset('productimages/' . $checkout->newproduct->photo) }});">
   ...
   <div class="containerr">
      <div class="main-cart mb-3 mb-md-5 pb-0 pb-md-5">
         <ul class="cart-page-items d-flex justify-content-center list-inline align-items-center gap-3 gap-md-5 flex-wrap">
            ...
            <li class="cart-page-item active">
               <span class="cart-pre-heading badge bg-primary cart-pre-number border-radius rounded-circle me-1" style="background-color: #68b7e2;"> 3 </span>
      ...
      <div class="order">
         ...
         <ul class="details list-inline">
            <li class="detail">ORDER NUMBER:<strong>{{$checkout->id}}</strong></li>
            <li class="detail">DATE:<strong>{{($checkout->created_at)->format('Y.m.d')}}</strong></li>
            <li class="detail">EMAIL:<strong>{{$checkout->email}}</strong></li>
            <li class="detail">TOTAL:<strong>{{$checkout->symbole}}{{$checkout->price}}</strong></li>
            <li class="detail">PAYMENT METHOD:<strong>{{$checkout->pay_type}}</strong></li>
         </ul>
      </div>
         ...
         <div class="col-lg-8">
            <section class="maintable">
               <table class="table table-border">
                  ...
                  <tbody>
                     <tr class="order_item">
                        <td>Bag Pack <strong class="product-quantity">×&nbsp;1</strong></td>
                        <td class="text-end">
                           <span class="amount">
                              <bdi><span class="">{{$checkout->symbole}}</span>{{$checkout->price}}</bdi>
                           </span>
                        </td>
                     </tr>
                  </tbody>
                  <tfoot>
                     <tr class="order_item">
                        <th>Fee:</th>
                        <td class="text-end">
                           <span class="amount text-primary"><span class="">{{$checkout->symbole}}</span>{{$checkout->fee}}</span>
                        </td>
                     </tr>
                     <tr class="order_item">
                     ...
                     <tr>
                        <th>Total:</th>
                        <td class="text-end">
                           <span class="amount text-primary"><span class="">{{$checkout->symbole}}</span>{{$total}}</span>
                        </td>
                     </tr>
                  </tfoot>
               </table>
                     ...
                     <table class="table">
                        ...
                        <tbody>
                           <tr>
                              ...
                              <td class="last-name">{{$checkout->name}}</td>
                           </tr>
                           <tr>
                              <td class="label-name">Company</td>
                              <td class="seprator"><span>:</span></td>
                              <td class="last-name">{{$checkout->company}}</td>
                           </tr>
                           <tr>
                              <td class="label-name">City</td>
                              <td class="seprator"><span>:</span></td>
                              <td class="last-name">{{$checkout->city}}</td>
                           </tr>
                           <tr>
                              <td class="label-name">E-mail</td>
                              <td class="seprator"><span>:</span></td>
                              <td class="last-name">{{$checkout->email}}</td>
                           </tr>
                           <tr>
                              <td class="label-name">Phone</td>
                              <td class="seprator"><span>:</span></td>
                              <td class="last-name">{{$checkout->phonenumber}}</td>
                           </tr>
                        </tbody>
                     </table>
                  </div>
      ...
      <div class="text-center mt-4">
         <button onclick="window.print()" class="btn btn-primary">Print Order Summary</button>
      </div>
   </div>
</section>
@endsection